...
pub fn fetch_exec(cpu: &mut Cpu, mem: &mut MemMap) -> u8 {
    ...
    macro_rules! inc_HL { () => ({
        ...
        cpu.f &= CARRY_FLAG;
        if val & 0xfu8 == 0 { cpu.f |= HALF_CARRY_FLAG; }
    ...
    macro_rules! ccf { () => ({
        ...
        if cflag!() { cpu.f |= HALF_CARRY_FLAG } else { cpu.f &= !HALF_CARRY_FLAG }
        cpu.f ^= CARRY_FLAG
    }) }
    // SCF
    macro_rules! scf { () => ({
    ...
    // JP (HL)
    macro_rules! jp_HL { () => (
        cpu.pc = mem.lw(cpu.hl().get());
    ) }

    // CALL nn
    macro_rules! call_nn { () => ({
        cpu.sp -= 2;
        mem.sw(cpu.sp, cpu.pc);
        cpu.pc = get_nn!();
    }) }
    // CALL c, nn
    ...
    // LDI A, (HL)
    macro_rules! ldi_aHL { () => ({
        cpu.a = cpu.hl().get() as u8;
        inc_HL!();
    }) }
    // LDI (HL), A
    macro_rules! ldi_HLa { () => ({
        cpu.hl().set(cpu.a as u16);
        inc_HL!();
    }) }
    // LDD A, (HL)
    macro_rules! ldd_aHL { () => ({
